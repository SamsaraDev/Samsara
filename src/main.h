...
static const int LAST_POW_BLOCK = 16100;
static const int MODIFIER_INTERVAL_SWITCH = 16000;
...
public:
    ...
    {
        // Samsara: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CTxDB& txdb, uint64_t& nCoinAge) const;  // Samsara: get transaction coin age
...
public:
    ...
    // Samsara: block signature - signed by one of the coin base txout[N]'s owner
    ...
    // Samsara: two types of block: proof-of-work or proof-of-stake
    ...
    // Samsara: get max transaction timestamp
    ...
    bool AcceptBlock();
    bool GetCoinAge(uint64_t& nCoinAge) const; // Samsara: calculate total coin age spent in block
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    uint256 nChainTrust; // Samsara: trust score of block chain
    int nHeight;

    int64_t nMint;
    int64_t nMoneySupply;

    unsigned int nFlags;  // Samsara: block index flags
    enum  
    {
